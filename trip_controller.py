...
from models import db, Trip, Activity, Itinerary
...
def create_trip():
    ...
    new_itinerary = Itinerary(trip=new_trip)
...
def get_itinerary(trip_id):
    ...
    if not itinerary:
        return jsonify({"message": "Itinerary not found for this trip."}), 404

    activities = Activity.query.filter_by(itinerary_id=itinerary.id).all()
    result = [
        {
            'id': activity.id,
            'name': activity.name,
